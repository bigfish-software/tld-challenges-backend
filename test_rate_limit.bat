@echo off
echo Testing multiple submissions to verify rate limiting is disabled...

for /L %%i in (1,1,10) do (
    echo Testing submission %%i...
    curl -X POST "http://localhost:1337/api/submissions" ^
        -H "Authorization: Bearer " ^
        -H "Content-Type: application/json" ^
        -d "{\"data\":{\"runner\":\"TestRunner%%i\",\"challenge\":17,\"video_url\":\"https://www.youtube.com/watch?v=example%%i\",\"note\":\"Test submission %%i\",\"result\":\"02:15:%%i0\"}}" ^
        -s -o nul -w "Submission %%i: %%{http_code}\n"
)